...
.PHONY: $(LOCAL_MODULE)
$(LOCAL_MODULE): $(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so

# define INNER VARS
$(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so: INNER_MODULE := $(LOCAL_MODULE)
$(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so: INNER_LDFLAGS := -L$(GLOBAL_OUTPUT_SHARED) -L$(GLOBAL_OUTPUT_STATIC) $(LOCAL_LDFLAGS)
$(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so: INNER_LDLIBS := $(patsubst lib%,-l%,$(LOCAL_SHARED_LIBRARIES)) $(patsubst lib%,-l%,$(LOCAL_STATIC_LIBRARIES)) $(LOCAL_LDLIBS)
$(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so: INNER_OBJS := $(OBJS)
# add dependences for shared libs
$(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so: $(addprefix $(GLOBAL_OUTPUT_SHARED)/, $(addsuffix .so,$(LOCAL_SHARED_LIBRARIES)))
# add dependences for static libs
$(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so: $(addprefix $(GLOBAL_OUTPUT_STATIC)/, $(addsuffix .a,$(LOCAL_STATIC_LIBRARIES)))
# add dependences for objs
$(GLOBAL_OUTPUT_SHARED)/$(LOCAL_MODULE).so: $(OBJS)
	@echo [$(INNER_MODULE)] INSTALL $@ 
	@-mkdir -p $(GLOBAL_OUTPUT_SHARED)
	$(hide)$(CXX) -shared -o $@ $(INNER_OBJS) $(GLOBAL_LDFLAGS) $(INNER_LDFLAGS) $(GLOBAL_LDLIBS) $(INNER_LDLIBS)
